...
random_quartet.causal_quartet <- function(obj){ 
  ...
  if(space == "observables"){
    ...
    if(matchlq_given){ #if they input a latent quartet, use b, c, and d series
      ...
      #if they provided a yrange, make sure it isn't exceeded
      if(yrange_given){
        if(exceeds_bounds(y_b_treat,yrange) || exceeds_bounds(y_c_treat,yrange) || exceeds_bounds(y_d_treat,yrange)){
          warning("Input yrange is not compatible with ATE: Overriding provided yrange.")
          yrange_given <- FALSE
    ...
    }else{ #if they didn't provide a latent quartet to match
