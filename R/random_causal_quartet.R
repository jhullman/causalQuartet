random_causal_quartet <- function(ate, x, y=NULL, yrange=NULL, yoffset=0, obs=FALSE){
    ...
    if(max(y_d_treat) > yrange[2]){
      ...
      if(max(y_d_treat) > yrange[2]){
        if(yrange_given) {
          ...
          print(paste("set yrange[2] to", yrange[2], sep=" "))
        }
      }
    }
